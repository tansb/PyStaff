...
def rebin_spectrum(
    lamdas, flux, errors, pixel_weights, instrumental_resolution=None, skyspecs=None, c=299792.458
):
    """
    ...
    assert (lower >= lamdas.min()) & (
        upper <= lamdas.max()
    ), "Lower and upper limits must be within the wavelength ranges of the data"
...
def check_walker_starting_positions(p0, bounds):
    """
    Sometimes our random starting positions for the walkers scatter outside our prior ranges.
    This function checks which walkers are oustide the bounds limits and replaces them with the
    upper limut of the prior- so, for example, an age of 14.1 Gys becomes 14 Gyrs.

    Arguments:
        p0 (array): A 2D array of shape (N_parameters, N_walkers) of starting positions
        bounds (array): A 2D array of shape (N_parameters, 2). Each row of the array is (lower,
        higher) bounds for the repsective parameter.
    ...
    assert np.all((p0 > bounds[:, 0, None]) & (p0 < bounds[:, 1, None])), (
        "Some walkers are starting in bad places of parameter space!, "
        f"p0_max={p0.max(axis=1)}, p0_min={p0.min(axis=1)}, bounds={bounds}"
    )
...
def plot_fit(theta, parameters, fig=None, axs=None, color="b"):
    ...
    likelihood, [lams, temps, errors, specs, skies, emission_lines, polys, w] = lnlike(
        theta, parameters, ret_specs=True
    )

    # galaxy, noise, all_sky, weights, velscale, goodpixels, vsyst, interp_funct,
    # correction_interps, logLams, logLam_gal, fit_wavelengths=parameters
    ...
    if fig is None or axs is None:
        ...
        for i in range(len(fit_wavelengths)):
            ...
            inner_grid = gridspec.GridSpecFromSubplotSpec(
                ...
                width_ratios=[1, 1],
                height_ratios=[2, 1],
                subplot_spec=outer_grid[i // N_cols, i % N_cols],
                hspace=0.05,
            )
    ...
    for i, fit_range in enumerate(fit_ranges):
        ...
        for pair in zero_weights:
            ...
            high = pair[1]
            axs[0, i].axvspan(
                lams[low] / (np.exp(vel / c_light)),
                lams[high] / (np.exp(vel / c_light)),
                alpha=0.5,
                color="grey",
            )
            axs[1, i].axvspan(
...
def get_best_fit_template(theta, parameters, convolve=True):
    ...
    if positive_abundance_indices.size > 0:
        positive_only_correction = _get_correction(
            positive_only_interp, logLams, positive_abundance_indices, positive_abundances, age, Z
        )
    else:
    ...
    if general_abundance_indices.size > 0:
        general_correction = _get_correction(
            general_interp, logLams, general_abundance_indices, general_abundances, age, Z
        )
    else:
    ...
    template = np.exp(
        np.log(base_template)
        + general_correction
        + positive_only_correction
        + na_correction
        + carbon_correction
    )

    if convolve:
        template = P.convolve_gauss_hermite(
            template, velscale=velscale, start=[vel, sigma], npix=len(galaxy), vsyst=vsyst
        ).squeeze()
...
def get_linear_best_fit_template(theta, parameters, fit_class, convole=True):
    ...
    interp2 = si.interp1d(
        np.exp(fit_class.logLam_template), base_template, fill_value="extrapolate"
    )
...
def lnlike(theta, parameters, plot=False, ret_specs=False):
    ...
    if emission_lines is not None and emission_lines.shape[1] > 0:
    ...
    if emission_lines is not None:
        unconvolved_em_lines = (
            Hbeta_flux * emission_lines[:, 0]
            + Ha_flux * emission_lines[:, 1]
            + SII_6716 * emission_lines[:, 2]
            + SII_6731 * emission_lines[:, 3]
            + OIII * emission_lines[:, 4]
            + OI * emission_lines[:, 5]
            + NII * emission_lines[:, 6]
        )
        convolved_em_lines = P.convolve_gauss_hermite(
            unconvolved_em_lines,
            velscale=velscale,
            start=[vel_gas, sig_gas],
            npix=len(galaxy),
            vsyst=vsyst,
        ).squeeze()
    else:
    ...
    for fit_range in fit_ranges:
        this_fit_range_mask = (np.exp(logLam_gal) > fit_range[0]) & (
            np.exp(logLam_gal) < fit_range[1]
        )
