...
class SpectralFit(object):
    ...
    def __init__(
        ...
        if not len(np.unique(np.ediff1d(lamdas).round(decimals=8))) == 1:
            raise ValueError(
                "LAMDAS must be on a uniform wavelength grid, with no jumps or changes in wavelength spacing!"
            )

        if instrumental_resolution is not None:
            if (
                not np.unique(
                    np.array([flux.size, lamdas.size, noise.size, instrumental_resolution.size])
                ).size
                == 1
            ):
                raise ValueError(
                    "If INSTRUMENTAL_RESOLUTION is given, it must be the same length as FLUX, LAMDAS and NOISE"
                )
    ...
    def set_up_fit(self):
        ...
        self.lam_range_temp = np.round(
            np.array([self.lam_range_gal[0] - pad, self.lam_range_gal[1] + pad]), 1
        )
        ...
        if np.any(self.lam_range_temp < 3501) or np.any(self.lam_range_temp > 24997.58):
            raise ValueError(
                "The templates only extend from 3501 to 24997.58A! Lam_range_temp is {}".format(
                    self.lam_range_temp
                )
            )
    ...
    def rebin_spectra(self):
        ...
        (
            ...
            self.lin_noise,
            self.lin_weights,
            instrumental_resolution=self.instrumental_resolution,
            skyspecs=self.lin_skyspecs,
            c=SpectralFit.c_light,
        )
    ...
    def prepare_CVD2_interpolators(self):
        ...
        self.linear_interp, self.logLam_template = CvDTools.prepare_CvD_interpolator_twopartIMF(
            templates_lam_range=self.lam_range_temp,
            velscale=self.velscale,
            verbose=True,
            base_template_location=self.base_template_location,
            instrumental_resolution=self.instrumental_resolution,
        )
        self.correction_interps, self.log_lam_template = (
            CvDTools.prepare_CvD_correction_interpolators(
                varelem_template_location=self.varelem_template_location,
                templates_lam_range=self.lam_range_temp,
                velscale=self.velscale,
                elements=self.elements_to_fit,
                verbose=True,
                element_imf=self.element_imf,
                instrumental_resolution=self.instrumental_resolution,
            )
        )
    ...
    def get_emission_lines(self, vac_or_air="air"):
        ...
        self.emission_lines, self.line_names, self.line_wave = P.emission_lines(
            self.log_lam_template, self.lam_range_gal, self.FWHM_gal, vacuum=vacuum
        )
