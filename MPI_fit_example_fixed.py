...
fit = SpectralFitting.SpectralFit(
    lamdas,
    flux,
    errors,
    pixel_weights,
...
with MPIPool() as pool:
    ...
    p0[2 + n_general : 2 + n_general + n_positive, :] = np.abs(
        p0[2 + n_general : 2 + n_general + n_positive, :]
    )
    ...
    p0[p0 < bounds[:, 0, None]] = (
        bounds[np.any(p0 < bounds[:, 0, None], axis=1), 0] + 10 * np.finfo(np.float64).eps
    )
    # And the same for any values which are too low
    p0[p0 > bounds[:, 1, None]] = (
        bounds[np.any(p0 > bounds[:, 1, None], axis=1), 1] - 10 * np.finfo(np.float64).eps
    )

    assert np.all(
        (p0 > bounds[:, 0, None]) & (p0 < bounds[:, 1, None])
